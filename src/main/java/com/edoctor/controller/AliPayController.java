...
import com.edoctor.conf.AlipayConfig;
...
import com.edoctor.utils.AliPayJSONUtils;
...
public class AliPayController {
	...
	@ResponseBody
	public AliPayJSONUtils createOrder(@RequestParam(name = "productId") String productId, @RequestParam(name = "buyCounts") Integer buyCounts) throws Exception {
		...
		order.setOrderNum(orderId);
		order.setCreateTime(new Date());
		order.setBuyCounts(buyCounts);
		order.setOrderAmount(String.valueOf(Float.valueOf(p.getPrice()) * buyCounts));
		order.setOrderStatus(OrderStatusEnum.WAIT_PAY.key);
        orderService.saveOrder(order);
		return AliPayJSONUtils.ok(orderId);
	}
